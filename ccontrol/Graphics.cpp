...
std::string Rgb(uint32_t color, bool bg = false) {
...
void SetColorAt(HANDLE handle, const std::string& color, short x, short y)
{
    DWORD written;
    COORD start = {x, y};
...
void ResetAt(HANDLE handle, short x, short y)
{
    DWORD written;
    COORD start = {x, y};
...
void FillPrintCharAt(HANDLE handle, wchar_t c, short x, short y, uint8_t count)
{
    DWORD written;
